            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === item.id || (item.to === '/calendar' && activeTab === 'schedule')
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          <div className="flex items-end justify-between">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-slate-900">Clinical Schedule</h2>
              <p className="text-slate-500 mt-1">Full overview of your patient appointments.</p>
            </div>
            <button
            ...
            <div className="col-span-4 space-y-6">
              <div className="bg-white rounded-4xl border border-slate-200 shadow-sm p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold">Daily Agenda</h3>
                ...
                <div className="space-y-4">
                  {selectedDayAppointments.length > 0 ? (
                    selectedDayAppointments.map((app) => (
                      ...
                      >
                        ...
                        <div className="mt-4 flex items-center justify-between pt-4 border-t border-slate-50">
                           <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-bold uppercase">
                    ...
                    <div className="py-20 text-center flex flex-col items-center">
                      ...
                      <p className="text-sm font-bold text-slate-400 italic">No appointments for this day.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-indigo-600 rounded-4xl p-6 text-white shadow-xl shadow-indigo-200">
                 <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                       <p className="text-xs font-bold text-indigo-200 uppercase tracking-widest">Today's Schedule</p>
                       <p className="text-xl font-bold">{appointments.filter(a => isSameDay(new Date(a.date), new Date())).length} Sessions Total</p>
                    </div>
                 </div>
                 <div className="space-y-3">
                    <button className="w-full py-3 bg-white text-indigo-600 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all">Print Agenda</button>
                    <button className="w-full py-3 bg-indigo-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-indigo-400 transition-all border border-indigo-400">Sync Calendar</button>
                 </div>
              </div>
            </div>
          </div>
              ...
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg bg-white rounded-4xl overflow-hidden shadow-2xl p-8">
                  ...
                  <div className="space-y-8">
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 ml-1">Select Patient</p>
                      <div className="grid grid-cols-4 gap-4">
                        {MOCK_PATIENTS.map(patient => (
                          <button key={patient.id} onClick={() => setSelectedPatient(patient)} className={cn("flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all", selectedPatient.id === patient.id ? "border-primary bg-primary/5 shadow-sm" : "border-slate-100 bg-white hover:border-slate-200")}>
                            <img src={patient.avatar} alt={patient.name} className="size-12 rounded-xl object-cover" referrerPolicy="no-referrer" />
                            <span className={cn("text-[10px] font-bold text-center truncate w-full", selectedPatient.id === patient.id ? "text-primary" : "text-slate-600")}>{patient.name.split(' ')[0]}</span>
                          </button>
                        ))}
                      </div>
                    </div>
            ...
            {isLogoutModalOpen && (
              <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-sm bg-white rounded-4xl overflow-hidden shadow-2xl p-8 text-center" >
                  ...
                  <p className="text-slate-500 font-medium mb-8">Are you sure you want to exit the clinic management system?</p>
                  <div className="flex gap-4">
                    ...
                    <button onClick={() => { localStorage.clear(); navigate('/login'); }} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-bold shadow-lg shadow-rose-100 hover:bg-rose-700 transition-all flex items-center justify-center gap-2"><LogOut size={18} /> Logout</button>
                  </div>
                </motion.div>
              </div>
            )}
